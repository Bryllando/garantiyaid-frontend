...
const entrySource = readFileSync(new URL('../src/components/ui/use-motion-entry.js', import.meta.url), 'utf8')
...
test('motion layer keeps reduced-motion support and effect cleanup', () => {
  ...
  assert.match(entrySource, /export function useMotionEntry/)
  assert.match(entrySource, /prefers-reduced-motion: reduce/)
  assert.match(entrySource, /return \(\) => scope\.revert\(\)/)
})
