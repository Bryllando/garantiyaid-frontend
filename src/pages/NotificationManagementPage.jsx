...
function ReminderDialog({ distributions, onClose, onQueue }) {
  ...
  return <dialog ref={dialogRef} onCancel={onClose} onClose={onClose} className="m-auto max-h-[calc(100dvh-2rem)] w-[min(38rem,calc(100%-2rem))] overflow-y-auto rounded-2xl border border-line bg-white p-0 text-ink shadow-lg backdrop:bg-slate-950/55">
    <form onSubmit={submit} className="p-5 sm:p-7">
      ...
      <div className="mt-7 flex flex-col-reverse gap-3 border-t border-line pt-5 sm:flex-row sm:justify-end"><button type="button" disabled={isSubmitting} onClick={() => dialogRef.current?.close()} className="ga-btn-secondary">Cancel</button><button type="submit" disabled={isSubmitting || !distributionId} className="ga-btn-primary">{isSubmitting ? 'Queueing…' : sendAt ? 'Schedule reminders' : 'Queue reminders now'}</button></div>
    </form>
  </dialog>
}
